...
def test_numbered_lists(tmp_path):
    "numbered_lists='legal': every ol joins one shared multilevel numbering; Word counts document order"
    out = tmp_path/'t.docx'
    warns = convert(
        '<ol>\n<li>Confidentiality. First terms.\n'
        '<ol>\n<li>During employment.</li>\n<li>On termination.</li>\n</ol>\n</li>\n</ol>\n'
        '<p>An unnumbered paragraph between sections.</p>\n'
        '<ol start="2">\n<li>Proprietary Rights. More terms.\n'
        '<ol>\n<li>Pre-existing IP.\n<ol>\n<li>Deep item.</li>\n</ol>\n</li>\n</ol>\n</li>\n</ol>\n'
        '<ul>\n<li>a bullet stays a bullet</li>\n</ul>', out, numbered_lists='legal')
    teq(warns, [])
    teq(fast_checks(out), 'valid')
    lines = [' '.join(l.split()) for l in pandoc(out).splitlines()]
    for s in ('1. Confidentiality. First terms.', '(a) During employment.', '(b) On termination.',
        '2. Proprietary Rights. More terms.',   # counting continues across the interruption: document order
        '(a) Pre-existing IP.',                 # letters restart under the new level-1 item
        '(i) Deep item.', '- a bullet stays a bullet'): tt(s, lines, in_)
    num = zipfile.ZipFile(out).read('word/numbering.xml').decode()
    for s in ('(%2)', 'lowerLetter', '(%3)', 'lowerRoman'): tt(s, num, in_)
    teq(num.count('w:startOverride'), 9)   # only the bullet list restarts; the scheme instance never does

def test_numbered_list_refs(tmp_path):
    r"span and li ids inside numbered items are REF targets; \w yields the full-context number"
    out = tmp_path/'t.docx'
    warns = convert(
        '<ol>\n<li><span id="sec-conf">Confidentiality</span>. Terms.\n'
        '<ol>\n<li id="sec-conf-return">On termination, materials return.</li>\n</ol>\n</li>\n</ol>\n'
        '<p>See <a href="#sec-conf" data-ref=""></a> and <a href="#sec-conf-return" data-ref=""></a>.</p>',
        out, numbered_lists='legal')
    teq(warns, [])
    teq(fast_checks(out), 'valid')
    doc = zipfile.ZipFile(out).read('word/document.xml').decode()
    for s in (r'REF sec_conf \w \h', r'REF sec_conf_return \w \h', 'Section '): tt(s, doc, in_)
    # both bookmarks wrap content inside the numbered paragraphs, so \w resolves to the item's number
    assert doc.index('<w:numPr>') < doc.index('w:name="sec_conf"')
